...
v1.1.0。Windows、Ubuntu、macOS 共用 React 界面、PDF 阅读、Codex 服务、资料库与论文池；系统差异集中在已有的平台分支和打包配置中。没有复制一套 Linux 产品代码。
...
- 2026-09-08 完成坚果云 WebDAV 实际连接检查：Windows 成功读取远端论文池，远端提供强 ETag，本机与云端记录一致。用户随后确认 Ubuntu 同步正常，Windows / Ubuntu 双端验证成功。个人账户与凭据不进入仓库或 CI；其他 WebDAV 服务仍需分别验证。macOS 本轮未实机测试。

## Ubuntu 安装

目标为 Ubuntu 22.04／24.04 x64。优先使用 `.deb`；AppImage 为便携候选。安装包在 [Ubuntu 构建记录](https://github.com/SII-k7/Paper-Ocean/actions/workflows/linux-preview.yml) 的成功运行附件中，需登录 GitHub 下载。
