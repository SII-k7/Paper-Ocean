...
> **v1.2.4 · 论文辅助对话**：论文预览底部新增可折叠的 Luna max Fast 辅助窗口，与右侧主对话上下文独立、可同时问答，按论文保存历史和草稿。详见 [v1.2.4 更新说明](docs/v1.2.4.md)。

> **v1.2.3 · 问答连接提速**：修复首问失败重连造成的长时间等待，新旧对话直接使用 HTTP 流式连接；测试与限制见 [v1.2.3 更新说明](docs/v1.2.3.md)。

> **v1.2.1 · 一键更新**：Windows Setup 安装版与 Ubuntu `.deb` / AppImage 可在应用内下载安装、保存记录并重启。旧版本需最后手动安装一次；入口、迁移与支持范围见 [v1.2.1 更新说明](docs/v1.2.1.md)。

> **v1.1.1 · 新应用图标**：统一桌面图标、应用内 Logo 与浏览器图标。安装包及手动更新方式见 [v1.1.1 发布说明](docs/v1.1.1.md)；应用内「检查更新」会引导前往 GitHub 下载。
...
以下核心能力始于 v1.0.0，并保留在当前版本中。Windows 与 Ubuntu 安装包见 [构建下载说明](docs/ubuntu-paper-pool.md)；也可从源码运行或自行打包。旧版 v0.4.0 安装包不包含这些升级。历史变更见 [v1.0.0 迭代说明](docs/v1.0.0.md)，验证进度见 [实施记录](docs/implementation-progress.md)。
...
保留明暗主题及原有绿色强调色，布局选择滑块、搜索浮层和对话框使用短时过渡；键盘焦点清晰，遵循系统“减少动态效果”设置，PDF 与流式回答正文不反复做入场动画。通过本机 Codex 使用自己的 ChatGPT 登录，当前固定 **GPT-5.6 Luna · max**，优先启用目录中可用的 **Fast** 档位；可用性由当前客户端与账户决定，应用不要求填写 API Key。

### 8. 可观察的回答过程

准备论文、连接会话、准备页图、等待模型、思考、输出和重试分别显示状态；完成后保留首字与总耗时。同一会话在服务进程存活期间直接复用，论文分片以有界并发读取；保持原有文字预算、分片信任级别及页图证据。流式文本每 50 毫秒合并一次显示，多段回答按各自消息 ID 合并，结束、停止和保存时立即刷新；历史答案避免重复 Markdown 排版。
