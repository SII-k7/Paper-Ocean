...
export default function NotesPanel({ notes, highlights, papers, selectedId, selection, onSelect, onNew, onChange, onHighlightChange, onOpenAnchor, onOpenSource, onExport }: Props) {
  const [query, setQuery] = useState("");
  const [archived, setArchived] = useState(false);
  const [preview, setPreview] = useState(false);
  const [exporting, setExporting] = useState(false);
  const note = notes.find((item) => item.id === selectedId);
  const filtered = notes.filter((item) => Boolean(item.archivedAt) === archived && `${item.title}\n${item.body}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
  ...
  return <section className="notes-panel" aria-label="研究笔记">
    ...
    <select aria-label="选择笔记" value={filtered.some((item) => item.id === selectedId) ? selectedId : ""} onChange={(event) => { onSelect(event.target.value); setPreview(false); }}>
    ...
    <details className="highlight-library" open>
      <summary>原文高亮 · {highlights.filter((item) => Boolean(item.archivedAt) === archived).length}</summary>
      <small>每次选择同一页的文字，可向笔记逐次添加多页证据。</small>
      {highlights.filter((item) => Boolean(item.archivedAt) === archived).map((highlight) => <div className="note-anchor" key={highlight.id}>
