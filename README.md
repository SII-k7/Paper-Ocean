...
> **v1.3.0-preview.1 · 阅读工作台预览**：工具按需展开，选文明确附加到主/辅对话，引用可返回，回答可就地存入笔记；输入与流式回答改为局部更新。预览版供试读，不进入正式自动更新。变化、测量和回退见[预览说明](docs/v1.3.0-preview.1.md)。

> **v1.2.5 · 模糊搜索、问答提速与玻璃交互**：修复输入新标题时联想请求丢失，增加标题前缀与拼写容错备用检索；论文会话隔离无关记忆和扩展；浮动控件加入玻璃质感及可关闭的短时过渡。实测与范围见 [v1.2.5 更新说明](docs/v1.2.5.md)。
...
保留明暗主题及原有绿色强调色，浮动工具栏、辅助对话、搜索浮层和弹窗使用玻璃质感与短时过渡。顶部「界面效果」可以关闭玻璃或减少动态效果，也遵循系统相关设置；PDF 与流式回答正文保持清晰。通过本机 Codex 使用自己的 ChatGPT 登录，当前固定 **GPT-5.6 Luna · max**，优先启用目录中可用的 **Fast** 档位；可用性由当前客户端与账户决定，应用不要求填写 API Key。
...
产物写入 `release/`。推送 `v*` 标签后，GitHub Actions 会在 Windows、Ubuntu 与 Intel macOS runner 上构建、校验、生成 SHA-256 校验和；普通版本标签发布正式 Release，带预发布后缀的标签发布预览版本。手动运行工作流只上传可下载的 Actions artifacts。
